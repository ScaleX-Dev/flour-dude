  ...
  return (
    <div ref={emblaRef} className="overflow-hidden lg:hidden">
      <div className="flex gap-4">
        {items.map((item) => (
          <article key={item.id} className={`relative min-w-0 flex-[0_0_88%] rounded-card border border-borderColor p-6 ${item.cardTone}`}>
            <p className="pointer-events-none absolute left-4 top-0 font-display text-[72px] leading-none text-caramel/20">“</p>
            <p className="relative z-10 mt-6 font-display text-[15px] italic leading-relaxed text-textBody">{item.quote}</p>
