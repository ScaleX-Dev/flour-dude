...
export function WhatsAppFab() {
  ...
  const message = useMemo(() => {
    if (pathname.startsWith('/cakes')) {
      return whatsappMessages.customCake;
    }

    if (pathname.startsWith('/events')) {
      return whatsappMessages.b2b;
    }

    if (pathname.startsWith('/order')) {
      return whatsappMessages.cakeOrder;
    }
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="animate-wa-pulse fixed bottom-4 right-4 z-[9999] inline-flex h-14 items-center gap-2 rounded-full bg-[#25D366] px-4 text-sm font-semibold text-white shadow-2xl shadow-black/25 sm:bottom-4 sm:right-4 sm:h-auto sm:rounded-pill sm:px-5 sm:py-3"
